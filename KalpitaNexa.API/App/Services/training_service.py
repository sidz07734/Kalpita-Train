...
def _build_quiz_prompt(topic: str, context: str) -> str:
    return f"""
Generate 10-12 MCQ questions on the topic: "{topic}"

Use the following training content as your knowledge source:

{context}

If the context doesn't cover the topic well enough, supplement with general
knowledge about the topic in the context of corporate/tech training.
...
class TrainingService:
    ...
    async def save_quiz_result(self, user_id: str, user_email: str, topic: str, score: int, total_questions: int, tenant_id: str = None, app_id: int = None) -> Dict[str, Any]:
        if not self.training_manager:
            return {"success": False, "error": "Training manager not available."}
        return self.training_manager.save_quiz_result(user_id, user_email, topic, score, total_questions, tenant_id, app_id)
...
# ------------------------------------------------------------------
    ...
    def _gather_topic_context(self) -> str:
        """Read all summaries from SharePoint as context for the quiz prompt."""
        try:
            summaries = self.sp.read_all_summaries()  # returns dict: {folder_name: content}
            combined = "\n\n".join(
                f"--- {folder} ---\n{text}" for folder, text in summaries.items() if text
            )
            return combined[:20_000]  # cap to avoid token overflow
