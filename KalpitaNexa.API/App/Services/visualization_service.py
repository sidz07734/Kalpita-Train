...
class VisualizationService:
    ...
    async def generate_chart_data(self, query: str, chart_type: str, data_sources: List[str], max_results: int,app_id: Optional[int] = None, 
        ...
        analysis_result = await self._chat_service.process_chat(
            message=query, data_sources=data_sources,app_id=app_id,tenant_id=tenant_id,
            user_role=user_role,
            user_email=user_email,skip_chart_detection=True
        )
    ...
    async def generate_dashboard_data(self, query: str, data_sources: List[str], max_results: int,app_id: Optional[int] = None, 
        tenant_id: Optional[str] = None, 
        user_role: Optional[str] = None, 
        user_email: Optional[str] = None) ->Dict[str, Any]:
        """
      .     Orchestrates the generation of multiple charts for a dashboard.
        ...
        tasks = [
            self.generate_chart_data(req['chart_query'], req['chart_type'], data_sources, max_results,app_id, tenant_id=tenant_id,
                user_role=user_role,
