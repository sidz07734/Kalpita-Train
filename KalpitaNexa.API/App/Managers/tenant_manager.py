...
class TenantManager:
    ...
    async def upsert_tenant_db(self, tenant_id: Optional[str], name: str, email_domain: Optional[str], app_ids: List[int], feature_ids: List[int], user_id: str) -> Dict[str, Any]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                # UPDATED: Added @EmailDomain parameter
                cursor.execute(
                    "EXEC spUpsertTenant @TenantId=?, @TenantName=?, @ApplicationIds=?, @FeatureIds=?, @EmailDomain=?, @RequestingUserId=?",
                    (tenant_id, name, app_csv, feature_csv, email_domain, user_id)
                )
