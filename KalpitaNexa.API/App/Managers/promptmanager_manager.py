...
class PromptManagerManager:
    ...
    async def get_pending_approvals_db(self, tenant_id: str, start_date: str = None, end_date: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                ...
                for row in cursor.fetchall():
                    row_dict = dict(zip(columns, row))
                    req_date = row_dict.get('RequestDate')
                    
                    if req_date:
                        # Client-side filtering fallback: still format with 'Z'
                        row_dict['RequestDate'] = req_date.strftime('%Y-%m-%dT%H:%M:%S') + 'Z'
                        
                        # Server-side filtering logic
                        if filter_start and req_date < filter_start:
                            continue
                        if filter_end and req_date > filter_end:
                            continue
                    
                    approvals.append(row_dict)
                    
                total_pending = len(approvals)
                return {"success": True, "pending_approvals": approvals, "total_pending": total_pending}
        except Exception as e:
            logger.error(f"DB error in get_pending_approvals_db: {e}")
            return {"success": False, "error": str(e), "pending_approvals": [], "total_pending": 0}