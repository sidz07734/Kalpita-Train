...
class TrainingSharePointManager:
    ...
    def file_exists(self, day_folder_name: str, relative_path: str) -> bool:
        """Return True if a file exists inside the output/ folder of a Day folder."""
        path = f"{self._output_folder_path(day_folder_name)}/{relative_path}"
        try:
            resp = requests.get(
                self._item_url(path),
            ...
            logger.warning(f"file_exists check failed for '{path}': {e}")
            return False
    ...
    def output_file_exists(self, day_folder_name: str, file_name: str) -> bool:
        ...
        return self.file_exists(day_folder_name, file_name)
    ...
    def upload_short(self, day_folder_name: str, short_filename: str, video_bytes: bytes) -> str:
        """
        Upload a short MP4 to output/shorts/ in the Day folder.
        Returns the SharePoint URL of the uploaded file.
        e.g. output/shorts/short_1.mp4
        """
        drive_id = self._get_drive_id()
        shorts_path = f"{self._output_folder_path(day_folder_name)}/shorts/{short_filename}"
        url = f"{GRAPH_BASE}/drives/{drive_id}/root:/{shorts_path}:/content"
        resp = requests.put(
            url,
        ...
        resp.raise_for_status()
        logger.info(f"Uploaded short: {short_filename} to {day_folder_name}/output/shorts/")
        return resp.json().get("webUrl", "")

    def upload_cleaned_transcript(self, day_folder_name: str, content: str) -> None:
        """
        Upload transcript_cleaned.txt to output/ in the Day folder.
        Stores the cleaned transcript as plain text for reuse.
        """
        drive_id = self._get_drive_id()
        file_path = f"{self._output_folder_path(day_folder_name)}/transcript_cleaned.txt"
        ...
        resp.raise_for_status()
        logger.info(f"Uploaded transcript_cleaned.txt for {day_folder_name}")

    def read_cleaned_transcript(self, day_folder_name: str) -> Optional[str]:
        """
        Read transcript_cleaned.txt from output/ if it exists.
        Returns None if not found (will trigger re-cleaning).
        """
        drive_id = self._get_drive_id()
        file_path = f"{self._output_folder_path(day_folder_name)}/transcript_cleaned.txt"
        url = f"{GRAPH_BASE}/drives/{drive_id}/root:/{file_path}:/content"
    ...
    def download_video(self, day_folder_name: str) -> Optional[bytes]:
        """
        Download the .mp4 video file from the Day folder.
        Returns raw bytes, or None if no video found.
        """
        drive_id = self._get_drive_id()
        folder_path = f"{TRAINING_ROOT}/{day_folder_name}"
        url = f"{GRAPH_BASE}/drives/{drive_id}/root:/{folder_path}:/children"
        resp = requests.get(url, headers=self._auth_headers())
        resp.raise_for_status()
        ...
        download_url = video_item.get("@microsoft.graph.downloadUrl")
        if not download_url:
            return None

        logger.info(f"Downloading video: {video_item['name']} ({video_item.get('size', 0)//1024//1024}MB)")
    ...
    def download_short(self, day_folder_name: str, filename: str) -> Optional[bytes]:
        """Download a short video from output/shorts/ folder. Returns None if not found."""
        path = f"{self._output_folder_path(day_folder_name)}/shorts/{filename}"
        url = f"{self._item_url(path)}:/content"
        resp = requests.get(url, headers=self._auth_headers())
        if resp.status_code == 404:
            return None
    ...
    def read_shorts_metadata(self, day_folder_name: str) -> list:
        """
        Read shorts_metadata.json from output/shorts/ folder.
        Returns [] if not found.
        """
        path = f"{self._output_folder_path(day_folder_name)}/shorts/shorts_metadata.json"
        url = f"{self._item_url(path)}:/content"
        resp = requests.get(url, headers=self._auth_headers())
        if resp.status_code == 404:
            return []
        ...
        try:
            data = resp.json()
            return data.get("shorts", []) if isinstance(data, dict) else []
        except Exception as e:
            logger.warning(f"Failed to parse shorts_metadata.json for '{day_folder_name}': {e}")
            return []
