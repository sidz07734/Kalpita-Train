...
@router.post("/training/generate-shorts", response_model=TrainingApiResponse)
async def generate_shorts(
    request: GenerateShortsRequest,
    training_service: TrainingService = Depends(get_training_service),
):
    """
    Phase 2: Clean transcript, identify key segments via GPT,
    cut video clips with FFmpeg, upload shorts to SharePoint.

    Request body:
        { "day_folder_name": "Day 2 - AI training - Pavan" }
        { "day_folder_name": "Day 2 - AI training - Pavan", "force_reclean": true }
    """
    try:
        result = await training_service.generate_shorts(
            request.day_folder_name,
            request.force_reclean
        )
        return TrainingApiResponse(success=True, data=result)
    except FileNotFoundError as e:
        logger.warning(f"Generate shorts — file not found: {e}")
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        logger.error(f"Generate shorts failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Shorts generation failed. Check server logs.")
...
@router.get("/training/content", response_model=TrainingApiResponse)
async def get_training_content(
    day_folder_name: str,
    type: str,
    training_service: TrainingService = Depends(get_training_service),
):
    filename = "summary.docx" if type == "summary" else "qa.docx"
    text = training_service.sp.read_output_docx(day_folder_name, filename)
    return TrainingApiResponse(success=True, data={"text": text or ""})


@router.get("/training/shorts-metadata", response_model=TrainingApiResponse)
async def get_shorts_metadata(
    day_folder_name: str,
    training_service: TrainingService = Depends(get_training_service),
):
    shorts = training_service.sp.read_shorts_metadata(day_folder_name)
    return TrainingApiResponse(success=True, data={"shorts": shorts})
...
async def save_quiz_result(
    ...
    try:
        ...
        result = await training_service.save_quiz_result(
            ...
            questions_json=request.questions_json,   # ADD
...
@router.get("/training/shorts/stream", response_class=Response)
async def stream_short(
    ...
    """
    Stream a short video clip from SharePoint output/shorts/ folder.
    Used by the Angular chat video player.
    ...
    """
    try:
        video_bytes = await training_service.stream_short_file(day_folder_name, filename)
        return Response(
            content=video_bytes,
        ...
        logger.error(f"Short stream failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to stream short.")
