...
export class UserService {
  ...
  getAllApplicationsWithSettings(tenantId: string): Observable<GetAllApplicationsResponse> {
    // Append tenant_id as a query parameter
    const url = `${this.apiUrl}/applications/all?tenant_id=${encodeURIComponent(tenantId)}`;
  ...
  upsertRole(payload: UpsertRoleRequest): Observable<any> { // You can create a proper UpsertRoleResponse interface later
    // This should point to your new backend endpoint that handles the upsert.
    const url = `${this.apiUrl}/roles/upsert`;

    console.log(`[UserService] Upserting role with payload:`, payload);

    // A POST request is suitable for a complex operation like an upsert.
    return this.http.post<any>(url, payload, { headers: this.getAuthHeaders() }).pipe(
      catchError(this.handleError<any>('upsertRole'))
    );
  }
