...
export class TenantsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.isSuperAdminMode) {
      this.activeSection = 'tenants'; // Default to tenants list for super admin
      this.loadAllTenants();
  ...
  onTenantSelectionChange(shouldResetAppId: boolean = true): void {
    // When a tenant is selected in the form, filter the master application list
    // to find only the applications that belong to that tenant.
    this.filteredApplications = this.allApplications.filter(
      app => app.tenant_id === this.dataSourceForm.tenant_id
    );

    // Only reset the selected application if explicitly requested (usually on manual change)
    if (shouldResetAppId) {
  ...
  onDataSourceSaved(): void {
    if (!this.dataSourceForm.data_source_name.trim() ||
      !this.dataSourceForm.data_source_type.trim() ||
      !this.dataSourceForm.tenant_id ||
      !this.dataSourceForm.app_id) {
      this.showToastMessage('All fields are required.', true);
      return;
    }

    // Validate that data source name contains only letters and spaces
    const namePattern = /^[a-zA-Z\s]+$/;
    if (!namePattern.test(this.dataSourceForm.data_source_name.trim())) {
      this.showToastMessage('Data Source name should only contain letters and spaces (no numbers or special characters).', true);
      return;
    }
    ...
    const validConfigs = this.dsConfigurations.filter(c => c.config_key.trim() !== '' && c.config_value.trim() !== '');

    // Attach to payload
    this.dataSourceForm.configurations = validConfigs;

    this.userService.upsertDataSource(this.dataSourceForm).subscribe({
      next: (res) => {
        const action = this.dataSourceForm.data_source_id === 0 ? 'created' : 'updated';
        this.showToastMessage(`Data source ${action} successfully!`, false);
        this.closeCreateDataSourceModal();
        this.closeEditDataSourceModal();
        this.loadAllDataSources();
        this.isSubmittingDataSource = false;
      },
      error: (err) => {
        this.showToastMessage(err.error?.detail || `Failed to save data source.`, true);
        this.isSubmittingDataSource = false;
      }
    });
  }
  ...
  selectSection(section: string): void {
    ...
    else if (section === 'videoAnalytics') {
      this.isLoading = false;
    }
  ...
  // If we are in Super Admin "Applications" section, we pass an empty string (to get NULL in SQL)
  // If we want to filter by a specific tenant, we pass that ID.
  const idToPass = this.activeSection === 'applications' ? '' : (sessionStorage.getItem('TenantId') || '');

  this.userService.getAllApplicationsWithSettings(idToPass)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: (response) => {
        if (response.success) {
