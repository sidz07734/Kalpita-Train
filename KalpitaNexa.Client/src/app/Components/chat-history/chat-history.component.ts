...
export class ChatHistoryComponent implements OnInit, OnDestroy, OnChanges {
 ...
 // --- THIS IS THE FIX ---
  ...
  if (!this.selectedCategory || this.selectedCategory === 'tags' || this.selectedCategory === 'approvals') {
    // Tags: client-side only → re-apply filters
    if (this.selectedCategory === 'tags') {
      this.questions = [...this.allTaggedQuestions];
      this.applyFilters();
    }
    // Approvals: no refresh needed on filter change
    return;
  }
  ...
  if (['myhistory', 'favorites'].includes(this.selectedCategory) && !this.userId) {
  ...
  private loadPendingApprovals(): void {
    ...
    this.isLoading = true;
    this.chatHistoryService.getPendingApprovals(this.tenantId)
        ...
        .subscribe({
          next: (response) => {
            if (response && response.success) {
              console.log('✅ Question added successfully:', response.message_id);
