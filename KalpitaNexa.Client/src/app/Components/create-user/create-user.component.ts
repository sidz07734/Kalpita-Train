...
export class CreateUserComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.tenantId && this.appId && this.appName) {
      this.createUserForm.patchValue({ appName: this.appName });
      this.loadRolesForTenantAndApp(this.tenantId, this.appId);
      this.loadTenantEmailDomain(this.tenantId);
    } else {
      this.error = "Session information is incomplete (Tenant, App). Cannot load roles.";
      this.toasterService.error(this.error);
    }
  ...
  private loadTenantEmailDomain(tenantId: string): void {
    this.userService.getTenantById(tenantId)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
          if (res.success && res.tenant) {
            this.emailDomain = res.tenant.email_domain || '';
            this.createUserForm.get('userEmail')?.updateValueAndValidity();
          }
        }
      });
  }
  ...
  private emailDomainValidator(control: AbstractControl): ValidationErrors | null {
    ...
    if (!email || !this.emailDomain) {
      return null;
    }
    const domain = email.substring(email.lastIndexOf('@') + 1).toLowerCase();
    if (domain && domain !== this.emailDomain.toLowerCase()) {
