...
export class EditUserComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Removed direct call from ngOnInit to ensure it runs after async data loads
  }
  ...
  private loadInitialData(): void {
    ...
    applications: this.userService.getAllApplicationsWithSettings(this.tenantId!),
      ...
      next: ({ applications, roles, userDefaults }) => {
        ...
        // Apply restrictions after data is loaded and controls are potentially re-enabled/created
  ...
  private handleApplicationChanges(): void {
    this.editUserForm.get('application')?.valueChanges.pipe(
      ...
      // Re-apply restrictions after dynamic children (languages/models) are loaded
      this.checkUserRoleAndRestrictForm();
    });
  }
  ...
  toggleDropdown(type: 'roles' | 'languages' | 'models') {
    if (this.editUserForm.get(type)?.disabled) return;
