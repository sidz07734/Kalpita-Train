import { Component, ElementRef, ViewChild, AfterViewInit, OnInit, OnChanges, SimpleChanges, Output, EventEmitter, OnDestroy, Input, HostListener, NgZone } from '@angular/core';
import { DomSanitizer, SafeHtml, SafeUrl } from '@angular/platform-browser';
import { ChatService, Citation, ChatResponse, VisualizationRequest, VisualizationResponse, BraveSearchResponse, TranslationRequest, TranslationResponse, ChatRequest, UpdateMessageFeedbackResponse, ChangePasswordRequest, LanguageItem, ApplicationWithDataSources, ModelItem, UpdateChatFeedbackRequest, SseEvent } from '../../Services/chat.service';
...
interface Message {
  ...
  isChart?: boolean;
  chartData?: any;
  messageType?: 'text' | 'chart' | 'loading'|'quiz' | 'audio' | 'short';
  citations?: Citation[];
  ...
  shortsPage?: number;
  activeShort?: { url: string; title: string; topic: string; duration_seconds: number; short_number: number; };
...
export class ChatbotComponent implements AfterViewInit, OnInit, OnChanges, OnDestroy {
  ...
  private speakClickTimer: any = null;
  ...
  private skipStream: boolean = false;
  ...
  reopenDashboardData: any = null;
  activeDashboardMessageIndex: number = -1;
  ...
  public hasActiveDashboard: boolean = false;
  public lastDashboardQuery: string = '';
public showAuthDashboard: boolean = false;
  public authWidgets: any[] = [];
  public showLayoutModal: boolean = false;
  public availableLayouts: AuthLayoutConfig[] = [];

  public pendingWidgets: any[] = [];
  public pendingInsights: string = '';
  public pendingFollowUps: string[] = [];
  public lastDashboardContext: string = '';

  public currentLayoutStyle: any = {};
  public activeLayoutId: string = 'auto';
  public isDashboardChatMinimized: boolean = false;
  public dashboardChatHistory: Message[] = [];
  public dashboardQueryInput: string = '';
  public Math = Math;
  ...
  // --- NEW TEAMS LOCALSTORAGE HELPERS ---
  private persistTeamsAuthToLocalStorage(): void {
    ...
    localStorage.setItem('teams_auth_state', JSON.stringify(authData));
    localStorage.setItem('isTeamsMode', 'true');
  }

  private persistMessagesToLocalStorage(): void {
    if (!this.isTeamsMode) return;
    localStorage.setItem('teams_chat_history', JSON.stringify(this.messages));
  }

  private restoreTeamsStateFromLocalStorage(): boolean {
    const isTeams = localStorage.getItem('isTeamsMode') === 'true';
    if (!isTeams) return false;

    const authDataStr = localStorage.getItem('teams_auth_state');
    if (!authDataStr) return false;

    try {
      const authData = JSON.parse(authDataStr);
      if (new Date().getTime() > authData.expiry) {
        localStorage.removeItem('teams_auth_state');
        localStorage.removeItem('teams_chat_history');
        return false;
      }
      ...
      this.TenantId = authData.TenantId;
      this.appId = authData.appId;

      if (this.userEmail) sessionStorage.setItem('userEmail', this.userEmail);
      if (this.userRole) sessionStorage.setItem('userRole', this.userRole);
      if (this.userId) sessionStorage.setItem('userId', this.userId);
      if (this.userName) sessionStorage.setItem('userName', this.userName);
      if (this.TenantId) sessionStorage.setItem('TenantId', this.TenantId);
      if (this.appId) sessionStorage.setItem('appId', this.appId);
      if (authData.token) sessionStorage.setItem('access_token', authData.token);
      sessionStorage.setItem('isTeamsMode', 'true');
      
      this.isTeamsMode = true;

      const messagesStr = localStorage.getItem('teams_chat_history');
      if (messagesStr) {
        this.messages = JSON.parse(messagesStr);
        // Ensure timestamps remain Date objects
      ...
      console.error('Failed to restore Teams state from localStorage', e);
      return false;
    }
  }
  ...
  ngOnInit(): void {
    ...
    this.restoreTeamsStateFromLocalStorage();
    // --- RESTORE UI STATE IMMEDIATELY (Bug 2) ---
    this.isOpen = sessionStorage.getItem('chatbot_isOpen') === 'true';
    this.isMaximized = sessionStorage.getItem('chatbot_isMaximized') === 'true';
    this.showTenants = sessionStorage.getItem('chatbot_showTenants') === 'true';
    this.showAdminPanel = sessionStorage.getItem('chatbot_showAdminPanel') === 'true';
    this.showChatHistory = sessionStorage.getItem('chatbot_showChatHistory') === 'true';
    this.showTenantSelection = sessionStorage.getItem('chatbot_showTenantSelection') === 'true';
    
    // Ensure the window is open if a sub-view is active
    if (this.showTenants || this.showAdminPanel || this.showChatHistory || this.showTenantSelection) {
      this.isOpen = true;
      this.pushModalState();
    }
    ...
    this.headerAppName = sessionStorage.getItem('appName') || '';
    // this.establishAppId();
    ...
    // --- POPULATE TENANTS IF NEEDED (Bug 1 Refined) ---
    if (this.showTenantSelection && this.userEmail) {
  ...
  private initializeChatbot(): void {
    ...
    const welcomeMessage = `Welcome to KalpitaNexa. Your intelligent digital assistant is ready to provide accurate information, smart solutions, and seamless support. How may I assist you today?`;
  ...
  toggleChatbox(): void {
    ...
    sessionStorage.setItem('chatbot_isOpen', String(this.isOpen));
    if (this.isOpen) {
      this.unreadCount = 0;
  ...
  userAuthenticated(userData: { userEmail: string; userRole: string; userId: string; userName: string; TenantId: string; }): void {
    ...
    this.persistTeamsAuthToLocalStorage();
      ...
      next: (response) => {
        ...
        if (response.success && response.tenants && response.tenants.length > 0) {
          ...
          if (this.availableUserTenants.length === 1) {
            ...
            sessionStorage.setItem('chatbot_showTenantSelection', 'true');
            this.pushModalState();
          }
        } else {
  ...
  selectTenantForSession(tenant: UserTenantResponse): void {
    ...
    // 4. Push history state so user can go "Back" to picker (Bug 1)
    if (this.isSuperAdmin) {
      this.pushModalState();
    }
    this.persistTeamsAuthToLocalStorage();
    ...
    if (!this.isInitialized) {
      ...
      if (this.isTeamsMode) {
        try {
          ...
          console.warn("Teams SDK notifications failed:", e);
        }
      }
    }
  } 
...
if (lastMessage && (lastMessage.messageType === 'loading' || (lastMessage.name === 'KalpitaNexa' && lastMessage.message === ''))) {
  this.messages.pop();
}
  ...
  private async processBackendQuery(englishMessage: string, isRegeneration: boolean = false): Promise<void> {
    const appName = sessionStorage.getItem('appName')?.toLowerCase() || '';

    if (appName.includes('auth')) {
      this.isLoading = true;
      this.chatService.getDailyAuthInsights(englishMessage, parseInt(this.appId!), this.TenantId!).subscribe(res => {
        this.isLoading = false;
        if (res.success && res.widgets?.length > 0) {
          this.showDashboard = true;
          this.hasActiveDashboard = true;

          // PUSH REOPEN TILE TO CHAT HISTORY
          this.messages.push({
            name: 'KalpitaNexa',
            message: `📊 I've generated the dashboard analysis for you.`,
            isDashboardLink: true, // Special flag
            widgets: res.widgets,   // Store data for re-opening
            timestamp: new Date()
          });
        }
      });
    ...
    if (isDashboardRequest) {
      this.showDashboard = true;
      this.dashboardQuery = englishMessage;
      this.reopenDashboardData = null;
      this.hasDashboardData = true;
      this.isLoading = false;
      this.isProcessingRequest = false;
      this.messages.push({
        name: 'KalpitaNexa',
        message: `📊 I've generated the dashboard analysis for: "${englishMessage}"`,
        timestamp: new Date(),
        isDashboardLink: true,
        dashboardQuery: englishMessage
      });
      this.activeDashboardMessageIndex = this.messages.length - 1;

      // Force maximize for the dashboard
  ...
  name: 'KalpitaNexa', message: '', timestamp: new Date(),
  messageType: 'loading', citations: [], followUpQuestions: []
};
...
// Typewriter drain — pops 1 char every 18ms from the queue
this.typewriterQueue = [];
this.typewriterIndex = streamIndex;

const hasPendingOpenTag = (text: string): boolean => {
...
const drainQueue = () => {
  if (this.typewriterQueue.length === 0) {
    if (streamDone) {
      const citations = this.messages[streamIndex]?.citations || [];
      const finalText = citations.length > 0
        ? displayedText + this.formatCitationsAsBulletList(citations)
        : displayedText;
      this.messages[streamIndex] = {
        ...this.messages[streamIndex],
        messageType: 'text',
        message: finalText,
      ...
      this.currentSubscription = null;
      if (streamMessageId) {
        for (let i = streamIndex - 1; i >= 0; i--) {
          if (this.messages[i].name === 'User' && !this.messages[i].messageId) {
            this.messages[i].messageId = streamMessageId; break;
          }
        }
      }
      ...
      this.typewriterTimer = setTimeout(() => this.ngZone.run(drainQueue), 20);
    }
    return;
  }

  const isTableMode = displayedText.includes('<table') || displayedText.includes('<tr');
  let charsToConsume = 0;

  if (isTableMode) {
    // Buffer until a complete </tr> to avoid partial-HTML reflows
    let tempBuf = displayedText;
    let idx = 0;
    while (idx < this.typewriterQueue.length) {
      tempBuf += this.typewriterQueue[idx];
      idx++;
      if (tempBuf.endsWith('</tr>') && !hasPendingOpenTag(tempBuf)) {
        charsToConsume = idx;
        break;
      }
      if (idx >= 2000) { charsToConsume = idx; break; }
    }
    // No complete </tr> yet — keep buffering silently
    if (charsToConsume === 0 && !streamDone) {
      this.typewriterTimer = setTimeout(() => this.ngZone.run(drainQueue), 10);
      return;
    }
    if (charsToConsume === 0) charsToConsume = this.typewriterQueue.length;
  } else {
    // Normal text — batch 8 chars per tick
    charsToConsume = 1;
  }

  for (let i = 0; i < charsToConsume; i++) {
    displayedText += this.typewriterQueue.shift()!;
  }

  if (!hasPendingOpenTag(displayedText)) {
    this.messages[streamIndex] = {
      ...this.messages[streamIndex],
      messageType: 'text',
  ...
  this.typewriterTimer = setTimeout(() => this.ngZone.run(drainQueue), 18);
};

this.typewriterTimer = setTimeout(() => this.ngZone.run(drainQueue), 18);

this.currentSubscription = this.chatService.sendQueryStream(
  request.message, request.app_id, tenantId, clientId, userIdToken,
  request.data_sources, request.user_role, request.user_email, request.model
).pipe(takeUntil(this.destroy$)).subscribe({
  next: (event: SseEvent) => {
    this.ngZone.run(() => {
      if (event.type === 'error') {
        this.skipStream = false;
        this.messages.splice(streamIndex, 1);
        clearTimeout(this.typewriterTimer);
        this.typewriterQueue = [];
        this.isLoading = false; this.isProcessingRequest = false; this.currentSubscription = null;
        this.toastr.error(event.message, 'Error', { timeOut: 3000, positionClass: 'toast-top-right', progressBar: true });
        if (event.message.toLowerCase().includes('expired')) this.isCreditExpired = true;
        return;
      }
      if (event.type === 'meta') {
  this.messages[streamIndex] = { ...this.messages[streamIndex], citations: event.citations || [] };

  if (event.message_type === 'audio' || event.message_type === 'short' || event.message_type === 'shorts_list') {
    this.messages.splice(streamIndex, 1);
    clearTimeout(this.typewriterTimer);
    this.typewriterQueue = [];
    if (event.message_type === 'audio') {
      this.messages.push({
        name: 'KalpitaNexa', timestamp: new Date(),
        message: event.response || '',
        messageType: 'audio' as any,
        audioUrl: event.audio_url,
        audioTopic: event.audio_topic,
        audioSession: event.audio_session,
        messageId: this.generateMessageId(),
      });
    } else if (event.message_type === 'short') {
      this.messages.push({
        name: 'KalpitaNexa', timestamp: new Date(),
        message: event.response || '',
        messageType: 'short',
        ...
        shortSession: event.short_session,
        messageId: this.generateMessageId(),
      });
    } else if (event.message_type === 'shorts_list') {
      this.messages.push({
        name: 'KalpitaNexa', timestamp: new Date(),
        message: event.response || '',
        messageType: 'text',
        shortsList: event.shorts_list,
        shortSession: event.short_session,
        messageId: this.generateMessageId(),
      });
    }
  ...
  if (event.is_visualization && event.visualization_suggestion) {
    this.messages.splice(streamIndex, 1);
    clearTimeout(this.typewriterTimer);
    this.typewriterQueue = [];
    this.handleChartRequest(englishMessage, event.visualization_suggestion);
    return;
  }
}
      if (event.type === 'chunk') {
        if (this.skipStream) return;
        accumulatedText += event.content;
        for (const ch of event.content) {
          this.typewriterQueue.push(ch);
        }
        this.isLoading = false;
      }
      if (event.type === 'done') {
        ...
        streamDone = true;
        const langCode = this.selectedLanguage.split('-')[0];
        if (langCode !== 'en' && accumulatedText) {
          this.chatService.translateText({ text: accumulatedText, target_language: langCode, source_language: 'en' })
            .pipe(takeUntil(this.destroy$)).subscribe({
              next: (res) => {
                if (res.success && res.translated_text) {
                  this.typewriterQueue = [...res.translated_text].slice(displayedText.length);
                }
              }
            });
        }
  ...
  private handleChatResponse(response: ChatResponse, originalMessage: string, totalFiles: number = 0): void {
    ...
    } else if ((response as any).message_type === 'short') {
      const shortMsg: Message = {
        name: 'KalpitaNexa',
        message: (response as any).response || '',
        timestamp: new Date(),
        messageType: 'short',
        shortUrl: (response as any).short_url,
        shortTitle: (response as any).short_title,
        shortSession: (response as any).short_session,
        messageId: this.generateMessageId(),
      };
      this.messages.push(shortMsg);
      this.scrollToBottomImmediate();

    } else if ((response as any).message_type === 'shorts_list') {
      const shortsMsg: Message = {
        name: 'KalpitaNexa',
        message: (response as any).response || '',
        timestamp: new Date(),
        messageType: 'text',
        shortsList: (response as any).shorts_list,
        shortSession: (response as any).short_session,
        messageId: this.generateMessageId(),
      };
      this.messages.push(shortsMsg);
      this.scrollToBottomImmediate();
  ...
  private addMessage(sender: string, content: string, messageType: 'text' | 'chart' | 'loading' = 'text',
    ...
    this.scrollToBottomImmediate();
    this.persistMessagesToLocalStorage();
  }
  ...
  private isNearBottom(threshold: number = 120): boolean {
  if (!this.chatMessages || !this.chatMessages.nativeElement) return true;
...
// Gentle scroll used during streaming — only scrolls if user hasn't scrolled up
private scrollToBottomIfNear(): void {
  if (!this.chatMessages || !this.chatMessages.nativeElement) return;
  ...
  public closeChatbot(): void {
    this.isOpen = false;
    sessionStorage.removeItem('chatbot_isOpen');
    this.setMaximize(false);
    // this.showLogin = true;
  }
  ...
  autoAdjustTextarea(): void {
    ...
    // FIX: If expanded mode is active, set the fixed height and EXIT immediately.
    // This prevents the 'height = auto' reset which causes the flicker.
    ...
    // Normal auto-resize logic for non-expanded state
    textarea.style.height = 'auto';
    const newHeight = Math.min(textarea.scrollHeight, 200);
    textarea.style.height = `${newHeight}px`;

    // Only show scrollbar if content exceeds the max limit (200px)
...
saveQuizResult(msg: Message): void {
  ...
  const questionsSnapshot = qd.questions.map((q: any, i: number) => ({
    ...
    selected_answer: qd.selectedAnswers[i] || null,

    correct: qd.selectedAnswers[i] === q.correct_answer,
  ...
  this.http.post(
...

    {
      ...
      questions_json: JSON.stringify(questionsSnapshot),

      prompt_used: qd.topic
  ...
  onTemplateSelected(event: { template: TemplateOption; query: string }): void {
    ...
    // Replace the standard addMessage with a link message
    this.messages.push({
      name: 'KalpitaNexa',
      message: `🎯 Generated the ${event.template.name} dashboard.`,
      timestamp: new Date(),
      isDashboardLink: true,
      isPredefinedDashboard: true,
      dashboardQuery: event.query,
      templateOption: event.template
    });
  ...
  onDashboardStateGenerated(state: any): void {
    if (this.activeDashboardMessageIndex !== -1) {
      this.messages[this.activeDashboardMessageIndex].savedDashboardState = state;
    }
  }
  ...
  onDynamicDashboardSelected(query: string): void {
    ...
    this.messages.push({
      name: 'KalpitaNexa',
      message: `🎯 Generated dynamic dashboard.`,
      timestamp: new Date(),
      isDashboardLink: true,
  ...
  toggleChatHistory(): void {
    ...
    sessionStorage.setItem('chatbot_showChatHistory', String(this.showChatHistory));
  ...
  onHistoryClose(): void {
    ...
    sessionStorage.removeItem('chatbot_showChatHistory');
  ...
  toggleMaximize(): void {
    ...
    if (isEmbedded && !this.isMaximized) {
      ...
      sessionStorage.setItem('chatbot_isMaximized', String(this.isMaximized));
    }
  }
  ...
  public onSpeakClick(message: Message, event: MouseEvent): void {
    event.stopPropagation();
    if (event.detail === 2) {
      // Double-click — stop completely
      this.stopSpeaking();
      return;
    }
    // Single click — play / pause / resume
    this.speakMessage(message, event);
  }
  ...
  public speakMessage(message: Message, event: Event): void {
    ...
    this.synthesizer.speakSsmlAsync(
      ssml,
      result => {
        // Synthesis done generating — do NOT reset here, audio may still be playing
        if (result.reason !== SpeechSDK.ResultReason.SynthesizingAudioCompleted) {
          console.warn("Speech synthesis did not complete successfully.", result);
        }
      },
      err => {
  ...
  public stopSpeaking(): void {
    // 1. Close the synthesizer FIRST to stop audio streaming into buffer
    if (this.synthesizer) {
      try { this.synthesizer.close(); } catch (e) { }
      this.synthesizer = null;
    }

    // 2. Close the player to kill the audio buffer
    if (this.player) {
  ...
  toggleAdminPanel(): void {
    ...
    sessionStorage.setItem('chatbot_showAdminPanel', String(this.showAdminPanel));
    this.showOptionsMenu = false;
  ...
  onTenantsClose(): void {
    this.showTenants = false;
    sessionStorage.removeItem('chatbot_showTenants');
  ...
  handleSettingsClick(): void {
    this.showOptionsMenu = false;
    this.showTenants = true;
    sessionStorage.setItem('chatbot_showTenants', 'true');
    this.pushModalState();
  ...
  applyDataSourceSelection(): void {
    ...
    if (activeAppId) {
      ...
      if (app) {
        ...
        this.headerAppName = app.application_name;
  ...
  onPopState(event: any): void {
    ...
    if (this.showAdminPanel) {
      ...
      sessionStorage.removeItem('chatbot_showAdminPanel');
      ...
      console.log('✅ Handling Chatbot Closure / Back Navigation');
      
      if (this.isSuperAdmin && !this.showTenantSelection) {
        // Bug 1 Fix: Super Admin goes back from chat to Workspace Selection
        console.log('🔄 Returning to Workspace Selection (Super Admin)');
        this.showTenantSelection = true;
        sessionStorage.setItem('chatbot_showTenantSelection', 'true');
        this.isInitialized = false;
        this.TenantId = '';
        ...
        if (this.availableUserTenants.length === 0 && this.userEmail) {
          this.fetchUserTenants(this.userEmail);
        }
      } else {
      ...
      // Prevent navigation away from the page (handles Workspace modal and Login)
  ...
  private pushModalState(): void {
   ...
   public reopenDashboard(): void {
    if (this.hasActiveDashboard) {
      this.showDashboard = true;
    }
  }
  private applyAuthLayout(widgets: any[], layout: AuthLayoutConfig) {
    this.currentLayoutStyle = layout.containerStyles;
    this.activeLayoutId = layout.id;
    // Map widgets to dashboardData
    this.dashboardData = widgets.map(w => ({
      chart_id: Math.random().toString(),
      success: true,
      error: null,
      chart_data: w,
      original_query: this.lastDashboardQuery,
      chart_type: w.type || 'bar'
    }));
    this.showDashboard = true;
  }

  selectLayout(layout: AuthLayoutConfig) {
    ...
    if (this.pendingWidgets && this.pendingWidgets.length > 0) {
      // 1. Open the dashboard immediately
      this.applyAuthLayout(this.pendingWidgets, layout);

      // 2. Sync the internal dashboard chat history
      this.dashboardChatHistory = [
        { name: 'User', message: this.lastDashboardContext, timestamp: new Date() },
        {
          name: 'KalpitaNexa',
          message: this.pendingInsights,
          timestamp: new Date(),
          followUpQuestions: this.pendingFollowUps
        }
      ];

      // 3. PUSH PERSISTENT LINK TO MAIN CHAT
      // This is the link that stays in the chat bubble history
      const dashboardLinkMsg: Message = {
        name: 'KalpitaNexa',
        message: `📊 **Analysis Ready**: I've compiled the authorization data based on your request.`,
        timestamp: new Date(),
        isDashboardLink: true,  // Trigger for the special button in HTML
        widgets: [...this.pendingWidgets], // Store widgets in the message object
        cssClass: 'auth-success-msg'
      };

      this.messages.push(dashboardLinkMsg);
      this.scrollToBottom();
    }
  }

  reopenDashboardFromLink(msg: Message): void {
    if (msg.savedDashboardState) {
      // Pass the fully generated state (charts + layout) to skip regeneration
      this.reopenDashboardData = msg.savedDashboardState;
    } else {
      this.reopenDashboardData = null;
    }
    this.dashboardQuery = msg.dashboardQuery || '';
    this.showDashboard = true;
    this.wasMaximizedBeforeModal = this.isMaximized;
    this.setMaximize(true);
  }
  ...
  onTemplateSelectionCancelled(): void {
    ...
    getVisibleShorts(msg: Message): any[] {
    if (!msg.shortsList) return [];
    const page = msg.shortsPage || 0;
    return msg.shortsList.slice(page * 3, page * 3 + 3);
  }

  nextShorts(msg: Message): void {

    if (!msg.shortsList) return;

    const page = msg.shortsPage || 0;

    if ((page + 1) * 3 < msg.shortsList.length) {

      msg.shortsPage = page + 1;

    }

  }

  prevShorts(msg: Message): void {

    const page = msg.shortsPage || 0;

    if (page > 0) msg.shortsPage = page - 1;

  }
  playShortInPlayer(msg: Message, short: any): void {
    msg.activeShort = short;
  }
  asVideo(target: EventTarget | null): HTMLVideoElement {
