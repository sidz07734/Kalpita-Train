...
export class ChartComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges {
  ...
  @Output() maximizeChart = new EventEmitter<ChartConfig>();
   @Input() colorPalette: string = 'corporate';
  ...
  //     return null;
  //   }
  //   const themeColors = this.getThemeColors(this.colorPalette);
...
private processChartData(chartType: string) {
    if (!this.chartData.labels || !this.chartData.datasets) return null;

    const themeColors = this.getThemeColors(this.colorPalette); // Use selected palette
    ...
    return {
      ...
      datasets: this.chartData.datasets.map((dataset, index) => {
        ...
        let bgColors;
        if (['pie', 'doughnut', 'polarArea'].includes(chartType) && this.chartData.datasets.length === 1) {
            bgColors = data.map((_, i) => themeColors[i % themeColors.length]);
        } else {
            bgColors = [themeColors[index % themeColors.length]];
        }
        ...
        return {
          ...
          borderColor: bgColors, // Remove borders for clean look
  ...
  private getThemeColors(paletteName: string): string[] {
    const palettes: { [key: string]: string[] } = {
      'corporate': ['#4A6B7C', '#F2994A', '#A1B5C1', '#1A365D', '#6B8E23'], // Slate, Orange, Grey, Navy
      'ocean': ['#0077B6', '#00B4D8', '#90E0EF', '#03045E', '#48CAE4'],
      'sunset': ['#E63946', '#F4A261', '#E9C46A', '#2A9D8F', '#264653'],
      'emerald': ['#2D6A4F', '#219EBC', '#8ECAE6', '#FFB703', '#FB8500']
    };
    return palettes[paletteName] || palettes['corporate'];
  }
  private getChartOptions(chartType: string): any {
    ...
    const isMultiDataset = this.chartData.datasets && this.chartData.datasets.length > 1;
    if (!isNonCartesian) {
      (baseOptions as any).scales = {
        y: {
          beginAtZero: true,
          stacked: isMultiDataset,
