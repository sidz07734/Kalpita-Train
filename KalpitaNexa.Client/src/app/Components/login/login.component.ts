...
export class LoginComponent implements OnInit {
  ...
  setView(view: 'login' | 'forgotPassword' | 'resetPassword' | 'success', event?: Event): void {
    ...
    // Pre-fill the forgot password email with the login email if available
  ...
  onSubmit(form: NgForm): void {
    ...
    this.loginSubscription = this.chatService.login(this.email, this.password).subscribe({
      next: (response) => {
  ...
  onCancel(form?: NgForm): void {
    // Abort the ongoing API call if it exists
    ...
    // Reset the form validation states (removes red borders)
    if (form) {
      form.resetForm();
    }

    // NOTE: I removed `this.cancelLogin.emit();` so the user stays on the exact same page 
    // instead of closing the login component entirely.
  }
